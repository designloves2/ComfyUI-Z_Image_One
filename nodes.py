...
SUBFOLDER = "one-node-z-image-turbo"
        ...
        for jf in glob.glob(os.path.join(subf_dir, "*.json")):
            ...
            if not os.path.exists(dest):
                try:
                    ...
                    print(f"[ZImageTurbo] migrate sidecar {basename}: {e}")
            else:
                try:
                    os.remove(jf)
                except Exception:
                    pass
        if moved:
            print(f"[ZImageTurbo] Migrated {moved} metadata sidecar(s) to metadata/")
    except Exception as e:
        print(f"[ZImageTurbo] migrate_meta_sidecars error: {e}")
...
def _png_embed_meta(png_path, meta_dict):
    ...
    try:
        ...
        try:
            ...
            # Retry os.replace up to 15 times (≈3s total) before quietly giving up.
            # The JSON sidecar in metadata/ is already saved — PNG embedding is best-effort.
            import time
            replaced = False
            ...
            if not replaced:
                try:
                    ...
                    pass
                # Non-critical: JSON sidecar already saved
        except Exception:
            try:
                os.remove(tmp)
            except OSError:
        ...
        # Non-critical warning — JSON metadata is safely stored in metadata/ folder
        print(f"[ZImageTurbo] png_embed_meta skipped (JSON saved): {e}")
        return False
...
def _png_read_meta(png_path):
    ...
    try:
        ...
        print(f"[ZImageTurbo] png_read_meta error: {e}")
        return None
...
def _read_json_meta(image_path):
    ...
    for mp in (_meta_path(image_path), _meta_path_legacy(image_path)):
        ...
        try:
            ...
            print(f"[ZImageTurbo] read_json_meta error: {e}")
    return None
...
def _write_json_meta(image_path, meta_dict):
    ...
    if image_path.lower().endswith('.png') and os.path.exists(image_path):
        ...
        if ok_png:
            try:
                ...
                pass
            print(f"[ZImageTurbo] Meta embedded in PNG: {os.path.basename(image_path)}")
    ...
    try:
        ...
        print(f"[ZImageTurbo] write_json_meta error: {e}")
...
PromptServer.instance.routes.get("/z_image_turbo/workflow_t2i")(_serve_json("workflows/t2i_workflow.json"))
PromptServer.instance.routes.get("/z_image_turbo/workflow_i2i")(_serve_json("workflows/i2i_workflow.json"))
PromptServer.instance.routes.get("/z_image_turbo/workflow_inpaint")(_serve_json("workflows/inpaint_workflow.json"))
PromptServer.instance.routes.get("/z_image_turbo/workflow_outpaint")(_serve_json("workflows/outpaint_workflow.json"))
PromptServer.instance.routes.get("/z_image_turbo/workflow_controlnet")(_serve_json("workflows/controlnet_workflow.json"))
PromptServer.instance.routes.get("/z_image_turbo/workflow_face_redraw")(_serve_json("workflows/face_redraw_workflow.json"))
PromptServer.instance.routes.get("/z_image_turbo/workflow_remove_bg")(_serve_json("workflows/remove_bg_workflow.json"))


@PromptServer.instance.routes.get("/z_image_turbo/bgremoval_models")
...
async def get_config(request):
    ...
    return web.json_response({
        ...
        "save_subfolder": cfg.get("save_subfolder") or SUBFOLDER,
...
async def save_config_route(request):
    try:
        ...
        print(f"[ZImageTurbo] config save error: {e}")
        return web.json_response({"ok": False, "error": str(e)}, status=500)


@PromptServer.instance.routes.get("/z_image_turbo/gallery")
async def get_gallery(request):
    ...
    if favonly:
        ...
        subf_dir = os.path.normpath(_safe_resolve_output_path(output_dir, subf or SUBFOLDER))
        unique = []
        missing = set()
...
@PromptServer.instance.routes.post("/z_image_turbo/save_meta")
async def save_meta(request):
    try:
        ...
        print(f"[ZImageTurbo] save_meta error: {e}")
        return web.json_response({"ok": False, "error": str(e)})


@PromptServer.instance.routes.post("/z_image_turbo/update_meta")
async def update_meta(request):
    try:
        ...
        print(f"[ZImageTurbo] update_meta error: {e}")
        return web.json_response({"ok": False, "error": str(e)})


@PromptServer.instance.routes.get("/z_image_turbo/meta")
...
@PromptServer.instance.routes.post("/z_image_turbo/open_folder")
...
@PromptServer.instance.routes.post("/z_image_turbo/delete")
...
async def get_models(request):
    # Diffusion models (unet) — z-image-turbo variants
    try:
    ...
    # Model patches (e.g. Z-Image-Turbo Fun ControlNet Union)
    try:
        patches = _scan("model_patches", extensions=[".safetensors"])
    except Exception:
        patches = ["none"]

    # Face/person detector models — UltralyticsDetectorProvider expects "bbox/filename.pt"
    try:
        import folder_paths as fp
...
@PromptServer.instance.routes.get("/z_image_turbo/lora_triggers")
...
# Per-node server-side storage for the last generated image (keyed by unique_id).
# Avoids needing a visible widget to carry the value.
_last_images: dict = {}


@PromptServer.instance.routes.post("/z_image_turbo/set_last_image")
async def _set_last_image(request):
    """Called by JS after each in-node generation to record the result."""
    data = await request.json()
    uid = str(data.get("unique_id", ""))
    _last_images[uid] = data.get("image", {})
    return web.json_response({"ok": True})


@PromptServer.instance.routes.post("/z_image_turbo/free_memory")
async def _free_memory(request):
    """Unload models from RAM/VRAM via ComfyUI's built-in free endpoint."""
    import aiohttp
    try:
        # Forward to ComfyUI's own /free endpoint
...
@PromptServer.instance.routes.post("/z_image_turbo/copy_to_input")
async def _copy_to_input(request):
    """Copy an output image into the input directory so other nodes can load it."""
    ...
    new_name = f"zit_{_uuid.uuid4().hex[:8]}_{os.path.basename(filename)}"
...
NODE_CLASS_MAPPINGS = {"ZImageTurboOneNode": ZImageTurboOneNode}
NODE_DISPLAY_NAME_MAPPINGS = {"ZImageTurboOneNode": "Z-Image ONE (TJ)"}
